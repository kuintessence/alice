...
use syn::{
    parse2, punctuated::Punctuated, token::Comma, FnArg, ItemFn, Lifetime, LifetimeParam,
    ReturnType, Type,
};
...
pub fn internal_message_consumer(
    ...
    for input in inputs.iter() {
        match input {
            FnArg::Typed(x) => {
                if x.attrs
                    .iter()
                    .any(|x| x.path().get_ident().unwrap().to_string().eq("serialize"))
                {
                    let mut x = x.clone();
                    x.attrs = vec![];
    ...
    sig.asyncness = None;
    sig.generics.params.push(syn::GenericParam::Lifetime(LifetimeParam::new(
